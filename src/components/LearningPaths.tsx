...
import { GraduationCap, AlertTriangle, Clock, CheckCircle2, Loader2, BookmarkPlus } from 'lucide-react';
import { useLearner } from '../context/LearnerContext';
import type { Course, Enrolment } from '../../server/competency/types';
...
export default function LearningPaths() {
  ...
  const complete = async (enrolmentId: string) => {
    ...
    try {
      await fetch(`/api/enrolments/${enrolmentId}/complete`, { method: 'PATCH' });
      await load();
    } finally {
      setBusyCourseId(null);
    }
  ...
  return (
    <div className="h-full overflow-y-auto pr-1">
      <div className="mb-4">
        ...
        <p className="text-[10px] font-bold text-pink-500 uppercase tracking-widest mt-2">Recommended training, mapped to your identified competency gaps.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {recommendations.map((rec) => {
            ...
            return (
              <div key={rec.course.id} className="relative bg-zinc-950/80 border border-zinc-800/80 p-4 flex flex-col shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_20px_40px_-25px_rgba(0,0,0,0.9)]">
                ...
                <p className="text-xs text-zinc-500 mb-3 flex-1">{rec.course.description}</p>
